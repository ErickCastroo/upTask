...
export class PasswordMail {
  static passwordRecoveryEmail = async (user: IEmail) => {
    const htmlContent = `
    <!DOCTYPE html>
    <html lang="es">
      <head>
      ...
      <body style="background-color: #f3f0ff; font-family: 'Segoe UI', sans-serif; margin: 0; padding: 0;">
        <table width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td align="center" style="background-color: #7c3aed; padding: 2rem;">
              <h1 style="color: #fff; margin: 0; font-size: 1.8rem;">🔐 Recupera tu contraseña</h1>
              <p style="color: #ddd6fe; font-size: 0.9rem; margin-top: 0.5rem;">Hola ${user.name}, usa este código para restablecer tu contraseña</p>
            </td>
          </tr>
          <tr>
            <td align="center" style="padding: 2rem;">
              <table style="max-width: 480px; width: 100%; background: #fff; border-radius: 1rem; box-shadow: 0 5px 15px rgba(0,0,0,0.1); padding: 2rem;">
                <tr>
                  <td style="text-align: center;">
                    <h2 style="color: #6d28d9; font-size: 1.25rem; margin-bottom: 1rem;">Código de recuperación</h2>
                    <p style="color: #6b7280; font-size: 0.95rem; margin-bottom: 1.5rem;">
                      Introduce este código para continuar con el restablecimiento:
                    </p>
                    <div style="display: inline-block; background: linear-gradient(to right, #7c3aed, #a78bfa); color: white; font-size: 2rem; font-weight: bold; letter-spacing: 8px; padding: 1rem 2rem; border-radius: 12px; margin-bottom: 1.5rem;">
                      ${user.token}
                    </div>
    ...
    await transporter.sendMail({
      ...
      text: `Hola ${user.name}, tu código para recuperar la contraseña es: ${user.token}`,
      html: htmlContent,
    })
